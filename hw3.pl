...
/*I coudln't get questions 7, 9, 10, 11 to work 6 only outputs true*/
/*
...
point(3,2).
point(3,5).
line(P, Q) :- point(P, W), point(Q, W).
line(P, Q) :- point(W, P), point(W, Q).
vertical(Y) :- vertical(line(point(X,Y),point(X,Z))).
horizontal(X):- horizontal(line(point(X,Y),point(Z,Y))).
*/
...
/* Problem 0B (NOT GRADED):
    ...
    (Reference 3.1 example 3 Learn Prolog Now!
      http://www.learn/prolognow.orgl/pnpage.php?pagetype=html&pageid=lpn-htmlse9)
    */
...
/* Problem 2:
 Write a predicate init(All, BLst) that succeeds if Blst has all the items of ALL except the last item.  The items in BLst are in the same order as ALL.*/

/* Problem 2 Answer: */
%init_aux(E,[_|L]):-init_aux(E,L).
init([_L], [ ]).
...
is_increasing([]).
is_increasing([_L]).
is_increasing([X1,X2|L]):-X1=<X2, is_increasing(L).
...
element_at(E, [E|_], 0).
element_at(E, [_|T], N):- N>=0, N1 is N-1, element_at(E, T, N1).
...
merge([H|_], [], [H|_]).
merge([],[H|_],[H|_]).
merge([H|_],[H1|_],[H1,H|_]).
merge([H|T], [H1|T1],[H1,H|L] ):- merge(T, T1, L).
...
/* Problem 9:
   ...
   Exercise  3.4 (http:www.learnprolognow.orglpnpage.php?pagetype=html&pageid=lpn-htmlse11)

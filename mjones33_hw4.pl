...
/* Problem 3 Answer */
sumPartialR(1,[1]).
sumPartialR(N,[H,H2|T]) :- C is N-1, sumPartialR(C, [H2|T]), H is N + H2.
/*sumPartial(0, []).
sumPartial(N, [N|T]) :- sumPartial(N1, T),N is N1 + N,*/
...
/*
A)
	a) Let A and B be sets. A relation R from A to B is a subset of A × B. Given an
		ordered pair (x, y) in A × B, x is related to y by R, written xRy, if, and only
		if, (x, y) is in R. The set A is called the domain of R and the set B is called its
		co-domain.

	b) A function F from a set A to a set B is a relation with domain A and co-domain B
...
C)Not every relation is a function ex) Let A = {1,2,3} and B = {4,5,6} T = {(1,4),(1,4),(3,6)} is a relation but not a function
*Found in Discrete Mathematics with Applications fourth edition*
...
outgoing(X, L):- findall(C, edge(X, C), L).
incoming(X, L):- findall(C, edge(C, X), L).
/* Problem 6 Test */
...
/* Problem 7 Answer: */
/*
Homoiconic- when the program structure is similar as its syntax.
Yes.
Fully reflective - whent he program can modify its structure and behavior at runtime.
No.
*Wikipedia*
*/
...
computeS(Op, Arg1, Arg2, Res):- R =.. [Op,Arg1,Arg2], Res is R.
...
result([], []).
result([A|T], [R|T1]):- R is A, result(T, T1).
...
d(C, X, 0).
d(X, X, 1).
d(C*X, X, R):- number(C), R is C.
d(-U, X, R):- d(U, X, A), R is -(U).
d(U+V, X, R):- d(U, X, A), d(V, X, B), R is U + V.
d(U-V, X, R):- d(U, X, A), d(V, X, B), R is U - V.
d(U*V, X, R):- d(U, X, A), d(V, X, B), T is U* B, S is V *A,R is T + S.
d(U^N, X, R):- d(U, X, A), R is  N * A * U ^(N-1).
...
swap(leaf(A), leaf(A)).
/*two leaves*/
swap(tree(leaf(L), leaf(R)), tree(leaf(R), leaf(L))).

swap(tree(L,leaf(R)), tree(leaf(R), S)):- swap(L, S).

swap(tree(leaf(L),R), tree(S, leaf(L))):- swap(R, S).

swap(tree(L,R), tree(Rn, Ln)):- swap(R, Rn), swap(L, Ln).
